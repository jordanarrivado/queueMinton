...
const SelectInfo = ({ 
  ...
  useEffect(() => {
    const fetchCourtType = async () => {
      try{
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/users/${user.email}/areas/${selectedArea.name}/courtFeeType`);
  ...
  const handleSession = async (component, mode) => {
    ...
    if (displayData) {
      try {
        ...
        await axios.put(
          `${process.env.REACT_APP_API_URL}/users/${user.email}/areas/${selectedArea.name}/session`, 
