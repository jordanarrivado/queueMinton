...
const SelectInfo = ({ 
  ...
  useEffect(() => {
    const fetchCourtType = async () => {
      try{
        const response = await axios.get(`https://localhost:3001/users/${user.email}/areas/${selectedArea.name}/courtFeeType`);
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        const response = await axios.get(`https://localhost:3001/users/${user.email}/areas/${selectedArea.name}/sessions`);
  ...
  const handleSession = async (component, mode) => {
    ...
    if (displayData) {
      try {
        ...
        await axios.put(
          `https://localhost:3001/users/${user.email}/areas/${selectedArea.name}/session`, 
