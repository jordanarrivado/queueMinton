...
const Payment = ({
    ...
    const fetchPrices = async () => {
      try {
        ...
        const response = await axios.get(
          `https://localhost:3001/users/${user.email}/areas/${encodeURIComponent(localArea)}`
        );
  ...
  useEffect(() => {
    const fetchPlayerHistory = async () =>{
      try{
        ...
        const response = await axios.get(
          `https://localhost:3001/users/${user.email}/areas/${encodeURIComponent(localArea)}/sessions/${encodeURIComponent(sessionDate)}/playerHistory`
        );
  ...
  const handlePlayerPay = async (id, name, ball) => {
    ...
    try {
      ...
      const API_BASE_URL = `https://localhost:3001/users/${user.email}/areas/${localArea}/sessions/${sessionDate}/players/${id}/totalPaid`;
  ...
  const handlePayAllSelected = async () => {
    ...
    try {
      const localArea = encodeURIComponent(localStorage.getItem('LocalArea'));
      const sessionDate = encodeURIComponent(localStorage.getItem('Session'));
      const API_BASE_URL = `https://localhost:3001/users/${user.email}/areas/${localArea}/sessions/${sessionDate}/bulkPayment`;
  ...
  const handleSavePrices = async () => {
    try {
      ...
      console.log("Sending PUT request to:", `https://localhost:3001/users/${user.email}/areas/${encodeURIComponent(localArea)}/editCourtFeeType`);
  
      const response = await axios.put(
        `https://localhost:3001/users/${user.email}/areas/${encodeURIComponent(localArea)}/editCourtFeeType`,
      ...
      const updatePriceResponse = await axios.put(
        `https://localhost:3001/users/${user.email}/areas/${encodeURIComponent(localArea)}/updatePrices`,
        {
  ...
  const filteredPlayers = players.filter(player =>
   ...
   useEffect(() => {
      const fetchCourtType = async () => {
        ...
        if(localArea){
          try{
            const response = await axios.get(`https://localhost:3001/users/${user.email}/areas/${localArea}/courtFeeType`);
