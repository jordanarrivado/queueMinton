    ...
    const shadow = isChecked ? 'drop-shadow(0 0 8px rgba(0, 238, 255, 0.5))' : 'drop-shadow(0 0 8px rgba(34, 51, 68, 0.8))';
      ...
      useEffect(() => {
        ...
        const fetchData = async () =>{
          try{
            const response =  await axios.get(`${process.env.REACT_APP_API_URL}/users/${user.email}/schedules`);
      ...
      const addScheduleToBackend = async (selectedArea) => {
        try {
          ...
          const response = await axios.put(`${process.env.REACT_APP_API_URL}/users/${user.email}/schedules`, {
      ...
      const handleDelete = async (id) => {
        try {
          await axios.delete(`${process.env.REACT_APP_API_URL}/users/${user.email}/schedules/${id}`);
      ...
      const saveEditSchedule = async (selectedArea) => {
        try {
          ...
          const response = await axios.put(
            `${process.env.REACT_APP_API_URL}/users/${user.email}/schedules/${newSchedule._id}`,
            {
              title: newSchedule.title,
              description: newSchedule.description,
